...
import { getChefs } from 'apiSdk/chefs';
...
function DashboardPage() {
  ...
  const chefStat = useSWR<PaginatedInterface<ChefInterface>>(
    () => `/chefs`,
    () => getChefs({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Chefs',
      result: chefStat?.data?.totalCount,
      isLoading: chefStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
