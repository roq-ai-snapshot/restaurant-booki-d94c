...
import { getChefById } from 'apiSdk/chefs';
import { ChefInterface } from 'interfaces/chef';

function ChefViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ChefInterface>(
    () => (id ? `/chefs/${id}` : null),
    () =>
      getChefById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('chef', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/chefs/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Experience Years" text={data?.experience_years} />
                ...
                <FormListItem
                  label="Date Updated"
                  text={
                    data?.date_updated ? format(parseISO(data?.date_updated as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />
